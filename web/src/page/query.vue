<template>
  <div clas="home">
    ...
    <div class="servers" v-if="searchOptionValue == 'server'">
      ...
      >
        <p>
          <span>当前人数：{{ item.serverInfo }}</span>
          <span>当前模式：{{ item.mode }}</span>
          <span>当前地图：{{ item.currentMap }}</span>
    ...
    <div v-if="searchOptionValue == 'player' && player.id" class="player">
      <a-card :title="playerName" class="info">
        <p>
          <span>等级：{{ "lv." + player.rank }}</span>
          <span>游戏时间：{{ timePlayed }}</span>
        </p>
        <p>
          <span>击杀：{{ player.kills }}</span>
          <span>死亡：{{ player.deaths }}</span>
          <span>SPM：{{ player.scorePerMinute }}</span>
          <span>kpm：{{ player.killsPerMinute }}</span>
          <span>kd：{{ player.killDeath }}</span>
        </p>
        <p>
          <span>机器人服名单情况：</span>
          <span>存在异常的武器数量：{{ abnormalWeapons.length }}</span>
        </p>
        <p>
          <span :class="['status' + player.status, 'bfbanStatus']"
            >联Ban查询结果：<strong>{{ player.statusText }}</strong></span
          >
        </p>
        <a slot="extra" class="lef-btn">
          <a-button type="primary" @click="showModal"> 点我举报</a-button>
        </a>
      ...
      <a-card
        title="申述教程"
        class="info"
        v-if="abnormalWeapons.length && player.status != '1'"
      >
        <div class="reportTip">
          <p style="color: red">
            注意：你有数据异常武器，没有被添加全局白名单可能会被机器人服务器踢出
          </p>
          <p>
            先通过下方的武器列表找到你被标记异常的武器，然后根据你自身不同的情况选择不同的申述方式
          </p>
          <br />
          <p>如果是刷枪导致了异常</p>
          <p>
            <a
              :href="`https://battlefieldtracker.com/bfv/profile/origin/${player.userName}/gamereports`"
              target="_blank"
              >点击这里</a
            >进入到你的游戏记录页面，然后找到你的刷枪游戏记录，然后把链接保存下来
          </p>
          <br />
          <p>如果正常打打出了异常数据</p>
          <p>
            <a
              href="https://www.bilibili.com/read/cv4812329?from=search&spm_id_from=333.337.0.0"
              target="_blank"
              >点击这里</a
            >
            查看moss的教学，然后在打开moss的情况下录制一整局你被标异常的武器证据录像，并且数据达到了被标异常数据的水平以后。上传到任意可以随时阅览的在线视频网站。然后<a
              :href="`https://battlefieldtracker.com/bfv/profile/origin/${player.userName}/gamereports`"
              target="_blank"
              >点击这里</a
            >进入你的游戏记录页面，找到这把录制的游戏记录，然后把视频的在线观看地址，moss生成的zip包，游戏记录的链接一起保存下来
          </p>
          <br />
          <p>
            完成以上步骤以后，加入Q群 184305984
            ，把你刚刚保存的申述资料找群内管理员进行申述，申述成功以后在帮你添加全局白名单
          </p>
        </div>
      </a-card>
        ...
        <div class="reportTip">
          <p style="color: red">
            注意：你已经被<a
              ...
              target="_blank"
              >战地联ban</a
            >实锤，没有解除实锤状态可能会被机器人服务器踢出
          </p>
          <br />
          <p>
            <a
              :href="`https://bfban.com/#/cheaters/${player.originUserId}`"
              target="_blank"
              >点击这里</a
            >跳转到你的举报页面，查看被实锤理由
          </p>
          <br />
          <p>
            <a href="https://docs.qq.com/doc/DVXBYQUdvelJNbGJm" target="_blank"
              >点击这里</a
            >查看联ban申述示范
          </p>
          <br />
      ...
      >
        ...
        <span
          slot="status"
          slot-scope="status"
          :style="{ color: status ? 'red' : 'rgba(0, 0, 0, 0.65)' }"
          >{{ status ? "数据异常" : "" }}</span
        >
      </a-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    gameReport() {
      window.open(
        `https://battlefieldtracker.com/bfv/profile/origin/${this.player.userName}/gamereports`
      );
    },
    jumpBFban() {
  ...
  computed: {
    ...
    weaponsFilter() {
      ...
      const typeWeapons = weapons.filter((item) =>
        weaponsType[weaponType].includes(item.type)
      );
      return [
        ...typeWeapons.filter((item) => item.status),
        ...typeWeapons
